...
colorMapR = plt.get_cmap('Reds')
colorMapG = plt.get_cmap('Greens')
colorMapB = plt.get_cmap('Blues')
...
teacher = TeacherController("ALP-GMM", 0, ne, {'x':[.7,.9,3], 'y':[-.1,.06, 3]})

print("training environment name : " + env_name)
if not EVAL:
    run = wandb.init(
        project='ALP-GMM-Picking', 
        config={
        "ne": ne,
        "pick_len": pick_len,
...
params = env.env_params 
params_list = []
for i in range(ne):
    params_list.append(params[i])

for i in range(ne):
    teacher.log_env_params(env, params_list[i], index=i)
...
while time_step <= max_training_timesteps: ## prim_step
    ...
    for i, true_i in enumerate(true_indicies):
        ...
        if buf_envs[true_i].is_done():
            if len(buf_envs[true_i].rewards) == len(buf_envs[true_i].states):
                ...
                ps = teacher.record_train_episode(sum(buf_envs[true_i].rewards).detach().cpu().numpy())

                if time_step % 100 == 0:
                    wandb.log({"Params" : ps})

                teacher.set_env_params(env, index=true_i)
    ...
    if time_step % 200 == 0:
        print("Plotting now!")
        # Grab last 50 points, or all points, whichever's smaller
        o1_pts = np.array(list(env.o1_pts))
        o2_pts = np.array(list(env.o2_pts))
        o3_pts = np.array(list(env.o3_pts))

        # Take the last len(o1_pts) colors
        color_idx = np.linspace(0., .5, 50)[(50 - len(o1_pts)):50]

        # Plot o1 points in blue
        plt.scatter(o1_pts[:,0], o1_pts[:,1], c=color_idx, cmap=colorMapR)

        # Plot o2 points in green
        plt.scatter(o2_pts[:,0], o2_pts[:,1], c=color_idx, cmap=colorMapG)

        # Plot o3 points in red
        plt.scatter(o3_pts[:,0], o3_pts[:,1], c=color_idx, cmap=colorMapB)

        # Show the plot
        plt.savefig('plot.png')
        img = Image.open('plot.png')
        data = np.array(img)
        images = wandb.Image(img, caption="Object Center Locations for Most Recent Sample")
          
        wandb.log({"Obj_Centers": images})
