...
class TeacherController(object):
    def __init__(self, teacher, nb_test_episodes, ne, param_env_bounds, reward_bounds=None, seed=None, teacher_params={}):
        ...
        self.batch_params = [np.array([0,0,0,0,0,0])] * ne
    ...
    def record_train_episode(self, reward, index=0):
        ...
        # print(f"I got reward {reward} in environment {index}")
    ...
    def log_env_params(self, env, params, index=0):
        self.batch_params[index] = params

        self.env_params_train = self.batch_params

        # print("Updating with the following batch params:")
        # print(self.batch_params)
        # print("END BATCH PARAMS")


    def set_env_params(self, env, index=0):
        # batch_params = []
        self.batch_params[index] = copy.copy(self.task_generator.sample_task())
        assert type(self.batch_params[index][0]) == np.float32
        ...
        obs = env.reset_alp_gmm(self.batch_params)

        return self.env_params_train