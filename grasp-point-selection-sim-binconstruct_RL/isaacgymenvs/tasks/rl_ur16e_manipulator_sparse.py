...
class RL_UR16eManipulation(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.o1_pts = deque(maxlen=50)
        self.o2_pts = deque(maxlen=50)
        self.o3_pts = deque(maxlen=50)
        ...
        env_dict = {}
        shape = (self.num_envs,6)
        for i in range(shape[0]):
            env_dict[i] = np.array([np.random.uniform(0.57, 0.7, 1).reshape(
                        1,)[0], np.random.uniform(0.57, 0.7, 1).reshape(
                        1,)[0], np.random.uniform(0.57, 0.7, 1).reshape(
                        1,)[0],np.random.uniform(-0.15, 0.10, 1).reshape(1,)[0], 
                        np.random.uniform(-0.15, 0.10, 1).reshape(1,)[0],
                        np.random.uniform(-0.15, 0.10, 1).reshape(1,)[0]])

        self.env_params = env_dict
    ...
    def reset_alp_gmm(self, batch_params):
        # print(batch_params)
        env_dict = {}
        shape = (self.num_envs,6)
        for i in range(shape[0]):
            env_dict[i] = batch_params[i]
    ...
    def reset_init_arm_pose(self, env_ids):
        ...
        for env_count in env_ids:
            ...
            self.o1_pts.append([self.env_params[env_count][0], self.env_params[env_count][3]])
            self.o2_pts.append([self.env_params[env_count][1], self.env_params[env_count][4]])
            self.o3_pts.append([self.env_params[env_count][2], self.env_params[env_count][5]])

            offset_object1 = np.array([self.env_params[env_count][0], self.env_params[env_count][3], 1.45, 0.0,
                    0.0, 0.0])
            offset_object2 = np.array([self.env_params[env_count][1], self.env_params[env_count][4], 1.45, 0.0,
                    0.0, 0.0])
            offset_object3 = np.array([self.env_params[env_count][2], self.env_params[env_count][5], 1.45, 0.0,
                    0.0, 0.0])
